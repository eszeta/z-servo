...
class ServoAccessor : public PortTableAccessor {
 public:
  Error Init() override;
  ...
  Error GetServoId(uint8_t& id) override { return GetId(id); }
  Error GetReturnLevel(uint8_t& return_level) override {
    return GetStatusReturnLevel(return_level);
  }
  Error GetStatus(uint8_t& status) override;
  Error RecoveryEeprom() override;
  Error LoadEeprom() override;
  Error StoreEeprom() override;
  Error StoreEeprom(const uint8_t address, const uint8_t size) override;
...
//     uint8_t baudrate_index = 0;
//     for (uint8_t i = 0; i < sizeof(kBaudrateTable) /
//     sizeof(kBaudrateTable[0]);
//          i++) {
//       if (baudrate == kBaudrateTable[i]) {
//         baudrate_index = i;
//         break;
...
//    * @param option 选项
//    */
//   void SetOption(const uint8_t option) { WriteRegField(Regs::kOption,
//   option); }
...
//     WriteRegField(Regs::kMinStartupForce, static_cast<uint8_t>(force *
//     1000));
//   }
...
//     uint16_t threshold;
//     ReadRegField(
//         Regs::kCurrentProtectionThH, Regs::kCurrentProtectionThL,
//         &threshold);
...
//     WriteRegField(Regs::kTorqueProtectionTime, static_cast<uint8_t>(time /
//     10));
//   }
...
//                   Regs::kGoalPositionL,
//                   static_cast<uint16_t>(bit_utils::SignToTwos(position,
//                   15)));
//   }
...
//     WriteRegField(
//         Regs::kGoalTimeH, Regs::kGoalTimeL, static_cast<uint16_t>(pwm *
//         100));
//   }
...
//                   Regs::kGoalVelocityL,
//                   static_cast<uint16_t>(bit_utils::SignToTwos(velocity,
//                   15)));
//   }
...
//     uint16_t position;
//     ReadRegField(Regs::kPresentPositionH, Regs::kPresentPositionL,
//     &position); return bit_utils::SignToTwos(position, 15);
//   }
...
//     uint16_t velocity;
//     ReadRegField(Regs::kPresentVelocityH, Regs::kPresentVelocityL,
//     &velocity); return bit_utils::SignToTwos(velocity, 15);
//   }
...
//    * @param status 状态值
//    */
//   void SetStatus(const uint8_t status) { WriteRegField(Regs::kStatus,
//   status); }
...
//     WriteRegField(Regs::kMotorDirection, direction == Direction::CCW ? 1 :
//     0);
//   }
...
//     WriteRegField(Regs::kAdcCurrentFactor, static_cast<uint8_t>(factor /
//     100));
//   }
...
//     WriteRegField(Regs::kVelocityFilter, static_cast<uint8_t>(filter *
//     1000));
//   }
