...
class Servo {
 public:
  ...
  void SetMinStartupForce(float min_startup_force) { min_startup_force_ = min_startup_force; }
  void SetCWInsensitiveArea(uint8_t cw_insensitive_area) { cw_insensitive_area_ = cw_insensitive_area; }
  void SetCCWInsensitiveArea(uint8_t ccw_insensitive_area) { ccw_insensitive_area_ = ccw_insensitive_area; }
  void SetAngularResolution(uint8_t angular_resolution) { angular_resolution_ = angular_resolution; }
  void SetPositionCorrection(int16_t position_correction) { position_correction_ = position_correction; }
  ...
  void SetTorqueProtectionTime(uint16_t torque_protection_time) { torque_protection_time_ = torque_protection_time; }
  void SetOverloadTorque(float overload_torque) { overload_torque_ = overload_torque; }

  void SetMotorDirection(Direction motor_direction) { motor_direction_ = motor_direction; }
  void SetSensorDirection(Direction sensor_direction) { sensor_direction_ = sensor_direction; }

  void SetTorqueEnable(bool torque_enable) { torque_enable_ = torque_enable; }
  void SetTargetAcceleration(int16_t target_acceleration) { target_acceleration_ = target_acceleration; }
  void SetTargetPosition(int16_t target_position) { target_position_ = target_position; }
  void SetTargetTime(int16_t target_time) { target_time_ = target_time; }
  void SetTargetPwm(float target_pwm) { target_pwm_ = target_pwm; }
  void SetTargetVelocity(int16_t target_velocity) { target_velocity_ = target_velocity; }
 ...
 private:
  ...
  PidController pos_pid_{
      {.kp = 1.0f, .ki = 0.0f, .kd = 0.0f, .ff = 0.0f, .ramp = 0.0f, .limit = 0.0f, .deadband = 0.0f}};
