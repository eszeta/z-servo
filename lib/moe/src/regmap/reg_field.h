...
struct Trait : public moe::Noncopyable {
  template <typename T>
  struct AccessType {
  ...
  struct ConverterOf {
    ...
    using Type    = ::moe::regmap::Converter<value_t>;
  };
 ...
 */
template <typename ValueType, uint8_t Address, uint8_t Shift, uint8_t Bits>
struct Field : public moe::Noncopyable {
 public:
  ...
  static constexpr access_t kMask =
      static_cast<access_t>(moe::utils::bit_utils::CreateMask(kShift, kBits));
...
template <typename ValueType, typename HighFieldType, typename LowFieldType>
struct Merged2 : public moe::Noncopyable {
  ...
  static constexpr value_t GetValue(const high_access_t high_access,
    ...
    constexpr access_t high_mask =
        static_cast<access_t>(moe::utils::bit_utils::CreateMask(0, kHighBits));
    constexpr access_t low_mask =
        static_cast<access_t>(moe::utils::bit_utils::CreateMask(0, kLowBits));
...
template <typename ValueType>
struct Converter : public moe::Noncopyable {
  using value_t = ValueType;
