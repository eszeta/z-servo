...
Error Slave::LinkAccessor(PortRegMap *regmap) {
  regmap_ = regmap;
  return Error::kOk;
}
...
Error Slave::WriteRegs(const uint8_t address,
  ...
  CHECK(regmap_->WriteMultiple(address, data, size));
...
Error Slave::ReadDataHandler(const InstPacket &packet, const bool response) {
  ...
  uint8_t buffer[128];
  CHECK(regmap_->ReadMultiple(address, size, buffer));
  if (response) {
...
Error Slave::BulkReadHandler(const InstPacket &packet, const bool response) {
  ...
  for (uint8_t i = 0; i < block_count; i++) {
    ...
    if (id == target_id) {
      hit = true;
      response_idx = i;
      CHECK(regmap_->ReadMultiple(address, data_size, buffer));
    }
    parameter += 1;
  }
  if (response && hit) {
