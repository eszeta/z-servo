...
inline Error MmioPlain::Init(uint8_t* regs, const size_t size) {
...
inline Error MmioPlain::Write(const uint8_t address, const uint8_t* data,
                              const size_t size) {
...
inline Error MmioPlain::Read(const uint8_t address, const size_t size,
                             uint8_t* data) {
  VERIFY(address + size <= size_, Error::kInvalidArg);
  for (size_t i = 0; i < size; ++i) {
