...
class RegMap : public protocol::RegMap<RegMap, regmap::RegMapMmio> {
  ...
  * - Torque Enable: 扭矩使能控制（与 Bit3 行为相关）
   ...
   */
  uint8_t GetSecondaryId() {
    uint8_t secondary_id = 0;
    ReadRegField(ControlTable::kSecondaryId, secondary_id);
    return secondary_id;
  }
   ...
   */
  void SetSecondaryId(const uint8_t secondary_id) {
    WriteRegField(ControlTable::kSecondaryId, secondary_id);
  }

  /**
   * @brief 获取协议版本 (R/W)
   * @return protocol_version 协议版本
   *
   * 范围: 1-2
   *
   * 【功能说明】
  ...
  uint8_t GetProtocolVersion() {
    ...
    ReadRegField(ControlTable::kProtocolVersion, protocol_version);
   ...
   */
  void SetProtocolVersion(const uint8_t protocol_version) {
    WriteRegField(ControlTable::kProtocolVersion, protocol_version);
  }
  ...
  /**
   * @brief 获取已注册指令 (R)
   * @return registered_instruction 已注册指令
   *
   * 范围: 0-255
   *
   * 【功能说明】
  ...
  uint8_t GetRegisteredInstruction() {
    ...
    ReadRegField(ControlTable::kRegisteredInstruction, registered_instruction);
   ...
   */
  void SetRegisteredInstruction(const uint8_t registered_instruction) {
    WriteRegField(ControlTable::kRegisteredInstruction, registered_instruction);
  }
