...
HardwareSerial serial_inst(PA10, PA9);
...
void setup() {
...
#ifdef ARDUINO_ARCH_STM32
  motor_driver.InitDriver(PA0, PA2);
  motor_driver.InitCurrentSensor(PA3, 1000, 1500);

  wire_sensor.begin();
  angle_sensor.InitI2C(&wire_sensor);
#elif ARDUINO_ARCH_GD32
  motor_driver.InitDriver(PA0, PA2, PA1, PA3);
  motor_driver.InitCurrentSensor(PA3, 1000, 100);
  ...
  servo.LinkCurrentSense(&motor_driver);
  servo.Init();
